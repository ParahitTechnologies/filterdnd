...
require_once('common/connection.php');
		$query="SELECT c.client_id,c.client_name,c.company_name,c.address,c.mobile_no,c.address,c.city,c.state,c.website_name,e.t_upload,e.fix_upload,e.t_number,e.fix_number,e.f_api,e.r_api,e.api_key FROM client c left join emp_master e on c.client_id=e.emp_id WHERE e.emp_id='$user_id'";
...
function account_save()
   {
	...
	if(client_name=='')
     ...
     var url="account_save.php?client_id="+client_id+"&client_name="+client_name+"&mobile="+mobile+"&client_address="+client_address+"&company_name="+company_name+"&state="+state+"&city="+city+"&website="+website+"&api_key="+api_key;
...
<script type="text/javascript" src="js/k_query.js"></script>
   <script type="text/javascript" language="javascript">
  $(document).ready(function() {
      $("#rekey").val("<img src='images/loading.gif'/>");
      $("#rekey").click(function(event){
          $.post( 
             "rendom_key.php",
             function(data) {
                $('#api_key').val(data);
   ...
   <tr>
    <td align="center"><div align="center" style=" background:white;width:100%;height:40%;valign:middle;-moz-border-radius:0.6em;border:1px solid #EEEEFF; opacity:0.8; filter:alpha(opacity=80)"><table width="100%" border="0" align="center" style=" -moz-border-radius: 8px; font-family:Arial, Helvetica, sans-serif; font-size:12px; font-weight:bold "  cellpadding="2" cellspacing="1">
           ...
           <td colspan="4" align="center"> <h1><span style="color:black">Manage Account </span></h1></td>
              ...
              <tr>
                <td width="14%" align="right"><div class="tf">Account Id </div></td>
                <td width="40%"><input type="text" name="client_id" id="client_id" value="<?=$client_id;?>" readonly="readonly"/></td>
                <td width="15%" align="right"><div class="tf">Contact Person</div></td>
                <td width="31%">
		...
		?>
              ...
              <tr>
                <td colspan="2" align="right" valign="top"><table width="100%" border="0" cellpadding="0" cellspacing="0">
                  <tr>
                    <td width=<?=$w1;?> align="right"><div class="tf">API Remaining</div></td>
                    <td width=<?=$w2;?>><input type="text" name="r_api" id="r_api" value="<?=$r_api;?>" readonly="readonly" style="margin-left:<?=$m1;?>"/></td>
                    <td width=<?=$w3;?>>&nbsp;</td>
                  </tr>
                  <tr>
                    <td align="right"><div class="tf">API Key</div></td>
                    <td><input type="text" name="api_key" id="api_key" value="<?=$api_key;?>" style="margin-top:5px;margin-left:<?=$m1;?>"/></td>
                    <td align="left" valign="middle"><img id="rekey" src="../images/refresh.gif" border="0" style="margin-top:5px;cursor:pointer"/></td>
                  </tr>
                </table>
                </td>
